...
namespace Far
{
    class Program
    {
        ...
        {
            Console.BackgroundColor = ConsoleColor.Magenta; // color of background where cursor is set
            int index = 0; // index for cursor
            foreach (FileSystemInfo fileInfo in directory.GetFileSystemInfos()) //cycle to show info about content
            {
                if (index == cursor) 
                    Console.ForegroundColor = ConsoleColor.Gray; // paints chosen file into gray
                else
